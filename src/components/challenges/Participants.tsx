...
export function Participants({ challengeId }) {
  ...
  useEffect(() => {
    const getParticipants = async () => {
      ...
      const { currentUser, participants } = participantsList.reduce(
        (acc, p) => {
          if (p.user_id === user.id) acc.currentUser = p;
          else acc.participants.push(p);
          return acc;
        },
        { currentUser: null, participants: [] }
      );


      setParticipants(participants); // others
      setCurrentUser(currentUser);   // current user
    };
  ...
  return (
    <>
      ...
      <div className="mt-6">
        ...
        <HabitCalendar allLogs={currentUser.logs} startDate={currentUser.start_date} endDate={currentUser.end_date ?? null} onChange={getDetails} />
        {/* <ChallengeLogs challengeId={challengeId} /> */}
      </div>
    </>
  );
}
